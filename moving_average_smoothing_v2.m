...
y2 = conv(x, hNQ);         % y2[n] = x[n] * h[n] = x[n] * (h1[n] * hNQ[n]) = (x[n]*h1[n]) * hNQ[n] = x[n+1] * hNQ[n] 
...
%cach 4 dung ham filter
doLonKhung = 3;
b = (1/doLonKhung)*ones(1,doLonKhung); % he so phan tu
y4=filter(b,1,x); % he so tu so, he so mau so, vector tin hieu vao 1 chieu
figure(5);
hold on
subplot(1,1,1)
% ve do thi y4[n] vs. s[n]
plot(n,y4(1:L),'r-',n,s(1:L),'b-');
xlabel('Chi so thoi gian n');
ylabel('Bien do');
legend('y4[n]','s[n]');
title('3-points smoothed y4[n] vs. original signal s[n]');
% ve do thi xep chong y1[n] , y2[n] , y3[n] , y4[n] de test ket qua
figure(6)
hold on
plot(n,y1,'r-',n,y2(1:L),'b-',n,y3(1:L),'c-',n,y4(1:L),'d-');
xlabel('Chi so thoi gian n');
ylabel('Bien do');
legend('y1[n]','y2[n]','y3[n]','y4[n]');
title('cach1  vs cach2 vs cach 3 vs cach4');
