...
%mfcc cho tat ca cac khung cua tat ca cac file tin hieu > tao ra cac matric
[mfcc_a,mfcc_e,mfcc_i,mfcc_o,mfcc_u] = matrixMfccTotalFrameInTotalFile(array_a,array_e,array_i,array_o,array_u);

%ham tich vector dac trung cho 1 nguyen am boi nhieu nguoiw noi
[mfcc_kq] = CalFeaturedVectorForPeople(array_a);


% Create feature vector database
[db,idx_a,idx_e,idx_i,idx_o,idx_u] = CreateDB(array_a,array_e,array_i,array_o,array_u);
...
PlotDB(db);
num = PlotIdx(idx_a,idx_e,idx_i,idx_o,idx_u);

% Debuging
% testfile = ".\NguyenAmKiemThu-16k\41MVS\o.wav";
% [vow,m,dis] = Predict1(testfile, db);
% listing = dir(".\NguyenAmKiemThu-16k\23MTL");
%weight =100
...
%am duoc danh dau
global N;
     mfcc_frame = melcepst(sig, fs, 'M', N, floor(3*log(fs)), 0.03*fs, 0.01*fs);
end
...
function [db,idx_a,idx_e,idx_i,idx_o,idx_u] = CreateDB(array_a,array_e,array_i,array_o,array_u)
    global K;
    [mfcc_a,mfcc_e,mfcc_i,mfcc_o,mfcc_u] = matrixMfccTotalFrameInTotalFile(array_a,array_e,array_i,array_o,array_u);
...
function [accuracy] = Accuracy(true, predict)
    count = 0;
    for i=1:length(true)
        if true(i) == predict(i)
            count = count + 1;
