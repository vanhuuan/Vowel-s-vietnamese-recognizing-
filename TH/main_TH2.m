...
%Statistics_mfcc(array_a,array_e,array_i,array_o,array_u);
%Statistics_fft(array_a,array_e,array_i,array_o,array_u)
%Cal_fft(array_a,array_e,array_i,array_o,array_u);
Cal_mfcc(array_a,array_e,array_i,array_o,array_u);
%[res,maxi,mini,mean_acc] = MultiTest(maxloop,array_a,array_e,array_i,array_o,array_u);
function cm = Statistics_mfcc(array_a,array_e,array_i,array_o,array_u)
    global N;
    global K;
    for N = 13:13:39
        for K = 1:5
            ...
            correct = [];
            numOfVowel = 105;
            for j = 1:3 %Chinh xuong con 1 thoi la nhanh
                % Create feature vector database
                db = CreateDB1(array_a,array_e,array_i,array_o,array_u);
            ...
            saiso = std(correct);
            disp("Gia tri trung bình la "+giatritrungbinh+"%");
            disp("Sai so là "+saiso+"%");
            disp("Ti le chinh xac tu "+min(correct)+"% đen "+ max(correct)+"%");
        end
    end
end 

function cm = Statistics_fft(array_a,array_e,array_i,array_o,array_u)
    global N;
    global K;
    N = 1024 
    for K = 1:5
        ...
        for j = 1:3 %Chinh xuong con 1 thoi la nhanh
            % Create feature vector database
            db = CreateDB2(array_a,array_e,array_i,array_o,array_u);
            % Predict test signals using created database
            [predict, trueVal] = PredictAll2("..\NguyenAmKiemThu-16k", db);
            % Plot confusion matrix
            cm = confusionmat(trueVal, predict);
            confusionchart(cm, ["/a/", "/e/", "/i/", "/o/", "/u/"]);
            correct(j) = 0;
...
function cm = Cal_mfcc(array_a,array_e,array_i,array_o,array_u)
    global N;
    global K;
    disp("N= "+N+"; K= "+K);
    correct = [];
    ...
    for j = 1:3
        % Create feature vector database
        [db,idx_a,idx_e,idx_i,idx_o,idx_u] = CreateDB1(array_a,array_e,array_i,array_o,array_u);
        % Predict test signals using created database
        [predict, trueVal] = PredictAll("..\NguyenAmKiemThu-16k", db);
        % Plot confusion matrix
        cm = confusionmat(trueVal, predict);
        confusionchart(cm, ["/a/", "/e/", "/i/", "/o/", "/u/"]);
        correct(j) = 0;
        for i = 1:numOfVowelFile
            if trueVal(i) == predict(i)
                correct(j) = correct(j) + 1;
            end
        end
        correct(j) = correct(j) / numOfVowelFile*100;
        disp("Lan "+j+" co to le chinh xac(%) la "+correct(j));
        PlotDB(db);
    end    
    disp("Gia tri trung bình la "+mean(correct)+"%");
    disp("Sai so la "+std(correct)+"%");
    disp("Ti le chinh xac tu "+min(correct)+"% den "+ max(correct)+"%");
    drawStable('..\TinHieuHuanLuyen\NguyenAmU\22_u.wav');
end 

function cm = Cal_fft(array_a,array_e,array_i,array_o,array_u)
    global N;
    global K;
    N = 2048;
    K = 1;
...
% Accuracy tests
% res(1): array of accuracies
% res(2): array of correct predictions
% maxi(1): maximum accuracy with number of correct predictions = maxi(2)
% mini(1): minimum accuracy with number of correct predictions = mini(2)
% mean_acc: mean of accuracy
function [res,maxi,mini,mean_acc] = MultiTest(maxloop,array_a,array_e,array_i,array_o,array_u)
    res = zeros(2,maxloop);
    maxi = zeros(1,2);
    mini = zeros(1,2);
    ...
    for i=1:maxloop
        ...
        % Predict test signals using created database
        [predict, trueVal] = PredictAll(".\NguyenAmKiemThu-16k", db);
        [acc,count] = Accuracy(trueVal, predict);
        res(1,i) = acc;
        res(2,i) = count;
    end
...
function drawStable(fileName)   
    [x1,Fs1] = audioread(char(fileName));
     x1 = x1./max(abs(x1));
    [vowel,Fs1] = Speech_Silence(fileName);
    sig = findStable(vowel);
    n_sig = length(sig);
    for i = 1:length(x1)- n_sig
        sampleFirst = i;
        sampleLast = i + n_sig-1;
        sig_tg = x1(sampleFirst:sampleLast);
        arr(i) = sum(sig == sig);
        if (sum(sig_tg == sig)==n_sig)
            mark = i;
            break;
        end
    end
    mark_end = mark+n_sig-1;
    t = 0:1/Fs1:(length(x1)-1)/Fs1; %time of sample
    for i = 1 : length(x1)
        if (mark<=i && i<=mark_end)
           sig_out(i) = sig(i-mark+1); 
        else
           sig_out(i) =0;
        end
    end
    ...
    %-----------------
     n_vowel = length(vowel);
     for i = 1:length(x1)- n_vowel
        sampleFirst1 = i;
        sampleLast1 = i + n_vowel-1;
        sig_tg1 = x1(sampleFirst1:sampleLast1);
        if (sum(sig_tg1 == vowel)==n_vowel)
            mark1 = i;
            break;
        end
     end
     mark_end1 = i+n_vowel-1;
     
     bound_mark_start = mark1 /Fs1;
     bound_mark_end = mark_end1 / Fs1;
    ...
    subplot(2,1,1)
    plot(t,x1);
    xlabel('time');ylabel('Magnitude'); hold on;
    plot ([bound_mark_start bound_mark_start],[-1 1],'r'); hold on;
    plot ([bound_mark_end bound_mark_end],[-1 1],'r');
...
function [mfcc_frame]= mfcc(sig,fs)
    %matric cua vector dac trung cua tat ca cac khung cua doan tin hieu nguyen am duoc danh dau
    global N;
...
% Plotting number of elements in each cluster (k-mean)
function [num] = PlotIdx(idx_a,idx_e,idx_i,idx_o,idx_u)
    global K;
    num = zeros(5,K);
    for i=1:K
        for j=1:length(idx_a)
            if idx_a(j) == i
                num(1,i) = num(1,i) + 1;
            end
        end
        for j=1:length(idx_e)
            if idx_e(j) == i
                num(2,i) = num(2,i) + 1;
            end
        end
...
function [] = PlotDB(db)
    ...
    for i=1:5
        for j=1:K
            p(i) = plot(db(j,:,i),color(i)+".-");
        end
    end
    title("Vector dac trung");
    legend(p, "/a/","/e/","/i/","/o/","/u/");
    hold off;
end
...
function [db,idx_a,idx_e,idx_i,idx_o,idx_u] = CreateDB1(array_a,array_e,array_i,array_o,array_u)
    global K;
...
function db = CreateDB2(array_a,array_e,array_i,array_o,array_u)
    global K;
    [fft_a,fft_e,fft_i,fft_o,fft_u] = matrixFftTotalFrameInTotalFile(array_a,array_e,array_i,array_o,array_u);
    [idx_a,db(:,:,1),sumd_a,D_a] = kmeans(fft_a, K);
    [idx_e,db(:,:,2),sumd_e,D_e] = kmeans(fft_e, K);
    [idx_i,db(:,:,3),sumd_i,D_i] = kmeans(fft_i, K);
    [idx_o,db(:,:,4),sumd_o,D_o] = kmeans(fft_o, K);
    [idx_u,db(:,:,5),sumd_u,D_u] = kmeans(fft_u, K);
end
...
function [Frames,Frame_length,NumOfFrame] = seftFraming(y0,Fs)  
    ...
    Frame_length = floor(0.03*Fs); % tính số điểm có trong 1 khung tín hiệu 20ms 
...
function [arrayCoeff] = matrixFftTotalFrame(array)
    global N Fs;
    arrayCoeff = [];
     for i = 1:length(array)
        [output,fs] = Speech_Silence(array{i});
        sig = findStable(output);
        [Frames,Frame_length,NumOfFrame] = seftFraming(sig,Fs);
        fftOneFile = [];
        for j = 1:NumOfFrame
            window = hamming(length(Frames(j)));
            fft_frame = abs(fft(sig.*window,N));
            fft_frame = fft_frame(1:floor(length(fft_frame)/2));
            %fft_frame = log(fft_frame);
            fft_frame = reshape(fft_frame, 1, []);
            fftOneFile = [fftOneFile; fft_frame];
        end
        arrayCoeff = [arrayCoeff;fftOneFile];
    end
end

function [fft_a,fft_e,fft_i,fft_o,fft_u] = matrixFftTotalFrameInTotalFile(array_a,array_e,array_i,array_o,array_u)
    fft_a=matrixFftTotalFrame(array_a);      
    fft_e=matrixFftTotalFrame(array_e);      
    fft_i=matrixFftTotalFrame(array_i);      
    fft_o=matrixFftTotalFrame(array_o);      
    fft_u=matrixFftTotalFrame(array_u);      
end
...
% Predict vowel (1 signal) based on given database
function [vowel,euc_dis] = Predict2(filename, db)
    global K N;
    % Find mfcc vectors and their mean on 1 signal
    [output,fs] = Speech_Silence(filename);
    sig = findStable(output);
    ...
    current_fvector = mean(fftOneFile);
...
% Predict all test signals 
function [predict,trueVal] = PredictAll2(testfolder, db)
    % Read all files in testfolder
    % and return their true and predicted vowels (in numbers 1-5 for a,e,i,o,u respectively)
    dir_content = dir(testfolder);
    count = 0;
    ...
    for i=3:length(dir_content)
        ...
        for j=3:length(subdir_content)
            ...
            if filename == "a.wav"
                count = count + 1;
                trueVal(count) = 1;
                predict(count) = Predict2(testfolder + "\" + subdir_name + "\" + filename, db);
                continue
            end
            if filename == "e.wav"
                count = count + 1;
...
function [accuracy, count] = Accuracy(true, predict)
    count = 0;
    for i=1:length(true)
        if true(i) == predict(i)
            count = count + 1;
    ...
    accuracy = count/length(true)*100;
end